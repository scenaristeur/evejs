...
TransportManager.registerType = function (constructor) {
  const type = constructor.prototype.type;
...
TransportManager.prototype.load = function (config) {
  if (Array.isArray(config)) {
    return config.map(config => this.load(config));
  }
...
TransportManager.prototype.unload = function (transport) {
  ...
  if (_transport) {
    ...
    const index = this.transports.indexOf(_transport);
    if (index !== -1) {
...
TransportManager.prototype.getByType = function (type) {
  if (type) {
    ...
    return this.transports.filter(transport => transport.type === type);
  }
  else {
